...
class trading_signal(pair_selection):
    def __init__(self, std_threshold, sectorName, corr_threshold, p_value_threshold, coint_start_date, coint_end_date, trade_start, trade_end, num_days):
...
#         z_score[z_score < -self.std_threshold].plot(color = 'green') 
        ...
        for i in range(len(z_score)):
            #Selling ratio
            if z_score[i] > self.std_threshold: 
#                 print(f'Selling {test_pair[0]}, Buying {test_pair[1]}')
            ...
            #Buying ratio
            elif z_score[i] < -self.std_threshold:
#                 print(f'Selling {test_pair[0]}, Buying {test_pair[1]}')
                trading_table['z_score'][i] = z_score[i]
...
if __name__ == '__main__':
    trade = trading_signal(1.5, 'Information Technology', 0.9, 0.05, '2014-01-01', '2016-01-01', '2016-01-01', '2021-01-01', 90)
